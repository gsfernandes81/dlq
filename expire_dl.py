...
Checking it
-----------
``python3 expire_dl.py --self-test`` runs 20 checks against a server on
loopback. Nothing in it reaches the network.

The one worth not weakening is the first: **a file smaller than one CHUNK must
be fetched, not declined.** The runner hands a partial item a slice of exactly
what it still needs, so every such file asks for less than a chunk, and the
sub-chunk guard used to refuse it before opening a connection. Exit 75 with
nothing moved is "not tonight" to the runner, and a cap under a MiB counts no
stall against it, so no strike was taken and no attempt was spent: the item was
offered the same too-small slice again on every firing, for ever, saying only
that it was still queued.
"""
...
def _self_test() -> int:
    """Drive the downloader against a local server, offline.

    Every check here stands for a way of spending -- or failing to spend --
    metered data while looking like it worked. The module had no checks at all
    until a 15 KiB wheel sat in the queue reporting "0 B of 15 KiB" on every
    firing, so the first of them is the one that failure would have caught.
    """
    ...
    passed = failed = 0
    # Every fetch logs, and checks.sh shows only the last 25 lines of a failing
    # suite. Left on the real stdout, the item's own account of twelve
    # downloads would push the FAIL line off the top of what a person is shown.
    terminal = sys.stdout
    ...
    body = bytes((i * 7 + 11) % 251 for i in range(5 * CHUNK + 1234))
    small = body[: 15 * 1024]
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802 - http.server's spelling
            State.requests += 1
            State.last_range = self.headers.get("Range") or ""
            data = State.payload
            start, end = 0, len(data) - 1
            ranged = State.last_range.startswith("bytes=") and not State.ignore_range
            if ranged:
                first, _, last = State.last_range[6:].partition("-")
                start = int(first)
                if last:
    ...
    root = Path(tempfile.mkdtemp(prefix="expire-dl-check-"))

    def attempt(name: str, *, slice_bytes: int, hint: int, sha: str | None = None,
                url: str = "/f.bin") -> tuple[str, Path, Path]:
        """One firing of one item, in its own work directory."""
        work, out = root / name / "work", root / name / "out"
        os.environ.update(
            EXPIRE_WORK=str(work),
            EXPIRE_OUT=str(out),
            EXPIRE_SLICE_BYTES=str(slice_bytes),
            EXPIRE_TOTAL_BYTES=str(hint),
            EXPIRE_STOP_EPOCH="0",
            EXPIRE_RUN_ID="check",
        )
        outcome = fetch(base + url, "f.bin", expect_sha256=sha, total_hint=hint)
        return outcome, work / "f.bin.part", out / "f.bin"

    try:
        sys.stdout = io.StringIO()
        # 1. The failure this module was missing. A file smaller than one CHUNK
        #    is handed a slice of exactly what it needs, and must be fetched
        #    rather than declined on every firing for ever.
        State.payload = small
        outcome, part, dest = attempt("whole-small", slice_bytes=len(small),
                                      hint=len(small))
        check("a sub-chunk file whose slice covers it completes", outcome, COMPLETE)
        # Compared as a digest, never as the bytes: a failing check is shown
        # through `tail -25`, and two 15 KiB repr()s would scroll it away.
        check("and lands whole", _sha256(dest) if dest.exists() else None,
              _hashlib.sha256(small).hexdigest())

        # 2. The production shape: dlq declares EXPECT_BYTES with a 3% + fixed
        #    margin, so the slice is *larger* than the file. The real total has
        #    to come off the wire, or the item never reaches its own total and
        #    is delivered by nothing.
        inflated = int(len(small) * 1.03) + 4096
        outcome, part, dest = attempt("inflated-cap", slice_bytes=inflated,
                                      hint=inflated)
        check("an inflated cap still completes", outcome, COMPLETE)
        check("at the size the server stated",
              dest.stat().st_size if dest.exists() else None, len(small))

        # 3. The guard the exemption must not remove: a slice too small to
        #    finish the file is still not worth a connection, and must not open
        #    one.
        before = State.requests
        outcome, part, dest = attempt("sub-chunk-short", slice_bytes=1024,
                                      hint=len(small))
        check("a slice that cannot finish the file declines", outcome, DECLINED)
        check("without making a request", State.requests, before)

        # 4. Slicing and resuming. The offset is st_size, so a partial file
        #    truncated by anything at all resumes from where the bytes actually
        #    stop rather than from a remembered number.
        State.payload = body
        outcome, part, dest = attempt("sliced", slice_bytes=2 * CHUNK, hint=len(body))
        check("a big file takes its slice and comes back", outcome, PROGRESS)
        check("the slice is the size it was given", part.stat().st_size, 2 * CHUNK)
        with part.open("r+b") as handle:
            handle.truncate(2 * CHUNK - 100)
        outcome, part, dest = attempt("sliced", slice_bytes=len(body), hint=len(body))
        check("resuming asks from where the bytes stop",
              State.last_range.split("=")[1].split("-")[0], str(2 * CHUNK - 100))
        check("and the rest completes", outcome, COMPLETE)
        check("byte for byte", _sha256(dest), _hashlib.sha256(body).hexdigest())

        # 5. A wrong hash is a strike a person sees, never a silent re-download.
        State.payload = small
        outcome, part, dest = attempt("bad-hash", slice_bytes=len(small),
                                      hint=len(small), sha="0" * 64)
        check("a sha256 mismatch is fatal", outcome, FATAL)
        check("and the partial is thrown away", part.exists(), False)
        check("and nothing is delivered", dest.exists(), False)

        # 6. A server that ignores Range on a resume must not have a second
        #    slice appended to the first: that corrupts silently.
        State.payload = body
        outcome, part, dest = attempt("no-range", slice_bytes=2 * CHUNK,
                                      hint=len(body))
        check("first slice of the ignoring server", outcome, PROGRESS)
        State.ignore_range = True
        outcome, part, dest = attempt("no-range", slice_bytes=2 * CHUNK,
                                      hint=len(body))
        check("a 200 to a ranged request is not appended", outcome, DECLINED)
        check("the partial is discarded rather than corrupted", part.exists(), False)
        check("and the refusal is remembered", _load_meta(part).get("no_range"), True)
        State.ignore_range = False

        # 7. The hash that matches, on the whole file, delivered.
        State.payload = small
        digest = _hashlib.sha256(small).hexdigest()
        outcome, part, dest = attempt("good-hash", slice_bytes=len(small),
                                      hint=len(small), sha=digest)
        check("a verified file is delivered", outcome, COMPLETE)
        check("and the sidecar is cleaned up", _meta_path(part).exists(), False)
    finally:
        sys.stdout = terminal
        server.shutdown()
        server.server_close()
    ...
    print(f"expire_dl self-test: {passed} passed, {failed} failed")
